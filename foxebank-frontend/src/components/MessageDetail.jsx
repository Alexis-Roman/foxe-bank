...
export default function MessageDetail({ request, onAccept, onDecline, onClose }) {
  ...
  return (
    <Box
      ...
      position="relative" // needed for CloseButton positioning
    >
      ...
      <VStack spacing={4} align="stretch" flex="1">
        <Box bg="gray.700" p={4} borderRadius="md">
          ...
          <Text fontSize="lg" fontWeight="bold">
            {request.requester?.name} ({request.requester?.number})
          </Text>
        </Box>
      ...
      <HStack spacing={4} mt={6}>
        <Button flex="1" colorScheme="green" onClick={() => onAccept(id)}>
          Accept
        </Button>
        <Button flex="1" colorScheme="red" onClick={() => onDecline(id)}>
          Decline
        </Button>
      </HStack>
    </Box>
